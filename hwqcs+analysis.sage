...
broken = False
while not broken:
	print("generate one signature and analyze it ...")		
	s,b,c,e = sign(H,sk)
	c = R1(c)
	stats0 = [ZZ(0)]*k
	stats1 = [ZZ(0)]*k
	...
	#z0 = [p0s0[i] for i in range(len(p0s0)) if stats0[i]<12]
	z0 = [i for i in range(k) if stats0[i]<12]
	print(len(z0),min(s0ones),len([i for i in s0ones if i==min(s0ones)]))
	#z1 = [k+p0s1[i] for i in range(len(p0s1)) if stats1[i]<12]
	z1 = [k+i for i in range(k) if stats1[i]<12]
	print(len(z1),min(s1ones),len([i for i in s1ones if i==min(s1ones)]))
	#z013 = [p0s0[i] for i in range(len(p0s0)) if s0zeros[i]==13]
	z013 = [i for i in range(k) if stats0[i]==12]
	#z113 = [k + p0s1[i] for i in range(len(p0s1)) if s1zeros[i]==13]
