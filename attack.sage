...
u = sample(range(n), w_u)  # sample u poly
f = sample(range(n), w_f)  # sample f poly
c = sample(range(n), w_c)  # sample c polynomial
e = sample(range(n), w_e)  # sample e polynomial
...
for i in range(n):
    shifted_c_support = np.mod(vectorSum(c,i),n); # shift of c by 0,1,2,3,...
    corr_val = 0
